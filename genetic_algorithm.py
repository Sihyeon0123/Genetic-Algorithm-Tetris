...
class Genetic_algorithm:
    def __init__(self, goal_score, size):
        global SIZE
        ...
        for i in range(SIZE):
            individuals[i].fitness = tetris.main('AI', generation, individuals[i], goal_score, size)
        ...
        result_screen.main(individuals[0].fitness, individuals[0].hw, individuals[0].aw, individuals[0].clw, individuals[0].bw)
        # 레코드 저장 코드 
...
def start(goal=2000, size=50):
    Genetic_algorithm(goal, size)
